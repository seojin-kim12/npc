					...
					local function vanillaEvent(event)
						...
						elseif(event.phase == "ended" or event.phase == "cancelled") then 
							if(event.target.isFocus) then
								...
								--if문을 통해 조건을 확인한뒤
								if event.target.x < doughMold.x + 300 and event.target.x > doughMold.x - 300      
								...
								local s=1
										local function stickEvent(event)
											...
											elseif(event.phase == "ended" or event.phase == "cancelled") then 
												if(event.target.isFocus) then
													...
													--if문을 통해 조건을 확인한뒤
													if event.target.x < doughMold.x + 1000 and event.target.x > doughMold.x - 1000      
													and event.target.y < doughMold.y + 1000 and event.target.y > doughMold.y - 1000 then 
														s=s+1
										...
										stick1:addEventListener("touch", stickEvent)	
								-------------------------------------------

							end
						end

					vanillat:addEventListener("touch", vanillaEvent)
			----------------------------------------------
		end
	end

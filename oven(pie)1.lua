...
function scene:create( event )
	...
	oven[3] = display.newImageRect("이미지/연어파이/오븐/연어오븐_00(꺼짐).png", 200*4.2, 150*3.2)
	...
	--케이크 드래그--
	local cake = {}
	cake[1] = display.newImageRect("이미지/연어파이/덜익은파이.png", 488*0.8,296*0.8)
	cake[2] = display.newImageRect("이미지/연어파이/덜익은파이.png", 488*0.6,296*0.6)

	cake[1].x, cake[1].y = display.contentWidth*0.25, display.contentHeight*0.63
	...
	--케이크--
	local c = display.newImageRect("이미지/연어파이/덜익은파이.png", 488*0.8,296*0.8)
	c.x, c.y = display.contentWidth*0.25, display.contentHeight*0.63
	sceneGroup:insert(c)
					...
					if(event.target.isFocus) then
						...
						local function click(event)              
							...
							if j==4 then
								...
								c.alpha=0
								composer.removeScene( "oven(pie)1" )
								composer.setVariable("complete", true)
								local options={
									effect ="fade",
									time=400
								}
								composer.gotoScene("oven(pie)2",options)
							end
	...
	elseif (event.target.x-event.x)^2 + (event.target.y-event.y)^2 > 150^2 then
		if i == 1 then

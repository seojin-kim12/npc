...
function scene:create( event )
	...
	local background2 = display.newImage("이미지/배경/요리테이블_왼손만.png")
	background2.x = display.contentWidth / 2
	background2.y = display.contentHeight / 2
	sceneGroup:insert(background2)
	background2.alpha=0

	local doma = display.newImageRect("이미지/연어파이/도마.png",1100,550)
	doma.x = display.contentWidth / 2
	doma.y = display.contentHeight *0.505
	sceneGroup:insert(doma)


	local s={}
	s[1] = display.newImageRect("이미지/연어파이/연어파이재료/연어.png",750,300)
	s[2] = display.newImageRect("이미지/연어파이/연어파이재료/연어썰기/연어2.png",750,300)
	s[3] = display.newImageRect("이미지/연어파이/연어파이재료/연어썰기/연어3.png",750,300)
	s[4] = display.newImageRect("이미지/연어파이/연어파이재료/연어썰기/연어4.png",750,300)

	for i=1,4 do
		s[i].x=display.contentWidth /2
		s[i].y=display.contentHeight /2
		s[i].alpha=0
		sceneGroup:insert(s[i])
	end

    local salmon = display.newImageRect("이미지/연어파이/연어파이재료/연어.png",346,158)
    salmon.x=display.contentWidth *0.14
	salmon.y=display.contentHeight *0.11
	...
	local function catch( event )
		...
		elseif ( event.phase == "ended" or event.phase == "cancelled" ) then
			--볼의 범위 안에 객체가 들어가면
			if event.target.x < doma.x + 450 and event.target.x > doma.x - 450
				and event.target.y < doma.y + 300 and event.target.y > doma.y - 300 then
					--재료와 위치가 바뀜
	...
	local function click(event)		
		...
		if c2>=5 then 
			...
			knife.alpha=0
			s[4].alpha=0
			composer.removeScene( "salmon2" )
        	composer.setVariable("complete", true)
        	local options={
				effect ="fade",
				time=400
			}
       		composer.gotoScene("salmon3",options)
        end
    end
    for i=1,4 do
		s[i]:addEventListener("tap",click)
	end

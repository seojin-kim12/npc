...
function scene:create( event )
	...
	local pie=display.newImageRect("이미지/연어파이/파이_반죽7.png",860,718)
	pie.x=display.contentWidth /2
	pie.y=display.contentHeight /2
	sceneGroup:insert(pie)

	local pie2=display.newImageRect("이미지/연어파이/연어파이완성.png",860,718)
	pie2.x=display.contentWidth /2
	pie2.y=display.contentHeight /2
	pie2.alpha=0
	sceneGroup:insert(pie2)

	local pd=display.newImageRect("이미지/연어파이/잘린반죽더미.png",450,150)
	pd.x=display.contentWidth *0.15
	pd.y=display.contentHeight *0.13
	...
	pd:addEventListener("touch", catch)
	...
	local function next(event)
		...
		time=400
       	...
       	composer.gotoScene("salmon7",options)
    end

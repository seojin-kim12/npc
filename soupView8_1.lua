...
function scene:create( event )
	...
	--국자
	local ladle = display.newImageRect("이미지/당근스프/당근/국자_빈.png",526.5, 730.5)
	ladle.x = display.contentWidth *0.85
	ladle.y = display.contentHeight * 0.5
			...
			if(event.target.isFocus) then
				...
				event.target.y < pot.y + 500 and event.target.y > pot.y - 500 )then
					...
					audio.play(soundTable["dropSound"])
            			...
            			composer.gotoScene("soupView8_2")
						background.alpha = 0
						pot.alpha = 0
						ladle.alpha = 0
						recipe.alpha = 0
	...
	local function big (event)
	   ...
	   elseif (event.target.x-event.x)^2 + (event.target.y-event.y)^2 > 200^2 then
		  if i == 1 then
